      ...
      {list.length === 0 ? (
        ...
        >
          ...
          {list.map((item) => {
            ...
            return (
              <motion.li key={item.id} variants={itemVariants} className={cn('relative flex gap-3 sm:gap-4', bare ? 'py-1.5 sm:py-2.5' : 'py-2.5')}>
                <span
                  className={cn(
                ...
                <div className={cn('min-w-0 flex-1 rounded-[14px] border border-[rgba(249,223,50,0.08)] bg-[rgba(13,0,0,0.35)] transition-colors hover:border-[rgba(249,223,50,0.2)]', bare ? 'px-3.5 py-2.5 sm:px-4 sm:py-3' : 'px-4 py-3')}>
                  ...
                  <p className={cn('mt-0.5 text-[12.5px] leading-relaxed text-cream-muted', bare && 'line-clamp-2')}>{item.description}</p>
                </div>
              </motion.li>
            )
          })}
...
export function ActivityEmpty() {
  return (
    <div className="flex flex-col items-center px-4 py-10 text-center">
      <div aria-hidden className="animate-float relative mb-5">
